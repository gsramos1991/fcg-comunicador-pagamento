...
try
{
    ...
    .Enrich.WithProperty("Application", "FCG-Comunicador-Getway-Pagamento") // Identificador no New Relic
    .WriteTo.Console()
    .WriteTo.NewRelicLogs(
        endpointUrl: "https://log-api.newrelic.com/log/v1", // Use "https://log-api.eu.newrelic.com/log/v1" se sua conta for da Europa
        applicationName: "FCG-Comunicador-Getway-Pagamento",
        licenseKey: LicenseNewRelic // Guarde a chave no appsettings ou environment
    )
    .CreateLogger();
...
{
    Log.Information("Desligando a aplicação");
    Log.CloseAndFlush();
}
